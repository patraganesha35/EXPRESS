...
export default function AdminEarningsChart() {
  ...
  useEffect(() => {
    ...
    // Import getSocket dynamically to avoid circular dependencies or use it directly if imported
    import("@/lib/socket").then(({ getSocket }) => {
      const socket = getSocket();
      socket.on("admin-earnings-updated", fetchEarnings);

      return () => {
        socket.off("admin-earnings-updated", fetchEarnings);
      };
    });
  }, []);
