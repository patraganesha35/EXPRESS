...
import {
  ...
  ChevronUp, Clock, Zap,
  CheckCircle2, KeyRound, ArrowRight, ArrowLeft,
...
export default function DriverRidePage() {
  ...
  }, [booking?._id, booking?.status, userData?._id]);
  ...
  /* No active booking */
  if (!booking) return (
    <div className="h-screen w-full bg-zinc-950 flex flex-col items-center justify-center px-6 relative">
      <button 
        onClick={() => router.back()}
        className="absolute top-6 left-6 text-white bg-white/10 hover:bg-white/20 p-2 rounded-full transition-colors"
      >
        <ArrowLeft size={20} />
      </button>
      <motion.div
  ...
  return (
    <div className="h-screen w-full bg-zinc-100 flex flex-col lg:flex-row overflow-hidden">
      ...
      <div className="relative flex-1 h-full z-0">
        <button 
          onClick={() => router.back()}
          className="absolute top-4 left-4 z-[500] bg-white/95 backdrop-blur-sm p-2.5 rounded-full shadow-lg border border-zinc-100 text-zinc-700 hover:text-zinc-900 transition-colors"
        >
          <ArrowLeft size={20} />
        </button>
        <LiveRideMap
